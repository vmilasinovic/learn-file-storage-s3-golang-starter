...
func main() {
	...
	s3ClientRegion, err := config.LoadDefaultConfig(context.Background(), config.WithRegion(s3Region))
	if err != nil {
		log.Fatal("Unable to load the region of s3 bucket")
	}
	s3Client := s3.NewFromConfig(s3ClientRegion)
